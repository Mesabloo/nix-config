...
let emacs = (if withMacport then llvmPackages_6.stdenv else stdenv).mkDerivation (lib.optionalAttrs nativeComp
  ...
  patches = patches fetchpatch ++ lib.optionals nativeComp [
    (substituteAll {
      src =
        if lib.versionOlder version "29"
        then ./emacs/native-comp-driver-options-28.patch
        else ./emacs/native-comp-driver-options.patch;
  ...
  src =
    if macportVersion != null then
      ...
      fetchFromGitHub {
        ...
        repo = "emacs";
        rev = "json-rpc";
        sha256 = "01s6c29d2dggsk6088yibk3r9ifiz1xl9jbir2brcsllrba8cx4s";
      };
  ...
  buildInputs =
    [ ncurses libxml2 gnutls gettext jansson harfbuzz.dev ] # gconf
      ++ lib.optionals stdenv.isLinux [ dbus libselinux alsa-lib acl gpm ]
      ++ lib.optionals withSystemd [ systemd ]
      ++ lib.optionals withX
      [ xlibsWrapper libXaw Xaw3d cairo ] # gconf
